(function (global, factory) {
    ...
    var _createClass     = function () {
        ...
        mdDateTimePicker = function () {
             ...
             * @param  {moment}   init                                    [initial value for the dialog date or time,
             *   defaults to today] [@default = today]
             * @param  {moment}   past                                    [the past moment till which the calendar shall
             *   render] [@default = exactly 21 Years ago from init]
             * @param  {moment}   future                                  [the future moment till which the calendar shall
             *   render] [@default = init]
             * @param  {Boolean}  mode                                    [this value tells whether the time dialog will
             *   have the 24 hour mode (true) or 12 hour mode (false)] [@default = false]
             * @param  {String}   orientation = 'LANDSCAPE' or 'PORTRAIT'  [force the orientation of the picker @default =
             *   'LANDSCAPE']
             * @param  {element}  trigger                                  [element on which all the events will be
             *   dispatched e.g var foo = document.getElementById('bar'), here element = foo]
            ...
            _createClass(mdDateTimePicker, [{
                ...
                value: function toggle() {
                    ...
                    if (mdDateTimePicker.dialog.state) {
                        ...
                        if (this._type === 'date') {
                            ...
                            if(moment(this._dateSelected).get('date')!== moment().get('date')){
                                this._init = moment(this._dateSelected).hour(moment(this._init).get('hour')).minutes(moment(this._init).get('minute'));
                            }else{
                                var previousTime = this._dateSelected.clone();

                                this._init = moment(this._dateSelected).hour(moment(this._sDialog.tDate).get('hour')).minutes((parseInt((this._sDialog.tDate).get('minute') / 15, 10) * 15));
                                if(previousTime>this._init){
                                    this._init = moment().hour(moment(previousTime).get('hour')).minutes((parseInt((previousTime).get('minute') / 15, 10) * 15));
                                }
                            }
                ...
                value: function _buildDialog() {
                    ...
                    if (this._type === 'date') {
                        ...
                        this._addId(AM, 'AM');
                        AM.textContent = 'AM';
                        if(moment(dateSelected).get('date')===moment(new Date).get('date')&&moment(dateSelected).get('hour')>12){
                            this._addClass(AM,'disabled');
                        }
                        this._addId(PM, 'PM');
                        PM.textContent = 'PM';
                ...
                value: function _initHour() {
                    var hourView = this._sDialog.hourView,
                        ...
                        disabled = 'mddtp-picker__cell--disabled',
                    ...
                    if (this._mode) {
                        ...
                        if(moment(initDate).get('date') === moment(new Date()).get('date')) {
                ...
                value: function _initMinute() {
                    var minuteView = this._sDialog.minuteView,
                        minuteNow  = parseInt(this._sDialog.sDate.format('m'), 10),
                    ...
                    for (var i = 5, j = 10; i <= 60; i += 5, j += 10) {
                        ...
                        div.appendChild(span);
                        docfrag.appendChild(div);
                        if(moment(initDate).get('date')=== moment(new Date).get('date') && moment(this._sDialog.sDate).get('hour')=== moment(new Date).get('hour')){
                            var currentMinutes = moment(new Date).get('minute');

                            for(var k = parseInt(currentMinutes/15)-1;k>=0;k--){
                                if(k*15===i){
                ...
                value: function _addClockEvent() {
                    var me         = this,
                        ...
                        disabledClass = 'mddtp-picker__cell--disabled';
                    hourView.onclick = function (e) {;
                        var ifDisabled = e.target.parentElement.className.indexOf(disabledClass)>-1?true:false;
                        if(!ifDisabled){
